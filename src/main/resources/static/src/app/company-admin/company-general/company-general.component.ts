...
@Component({
  ...
  styleUrls: ['./company-general.component.css']
})
export class CompanyGeneralComponent implements OnInit {
  ...
  constructor( public uploadService:UploadService,
    ...
    this.company=this.userSessionService.getCompany();
  ...
  submit(){
    ...
    let uploads=[];
    if(this.aboutUsFileName != this.company.aboutUsImage)
    ...
    if(uploads.length){
      Observable.forkJoin(uploads).subscribe((data:any[])=>{
        if(data.length && this.aboutUsFileName != this.company.aboutUsImage){
